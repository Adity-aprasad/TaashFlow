...
export function Dashboard({ room, players, currentPlayer }: DashboardProps) {
  ...
  const [roundHistory, setRoundHistory] = useState<Array<{ round: number; bets: BetRow[] }>>([])
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'scores', filter: `room_id=eq.${room.id}` },
        () => { fetchData() }
      )
      .subscribe()
  ...
  async function fetchData() {
    ...
    if (rounds && rounds.length > 0) {
      const history: Array<{ round: number; bets: BetRow[] }> = []
      for (const round of rounds) {
        ...
        history.push({ round: round.round_number, bets: (bets || []) as BetRow[] })
      }
  ...
  roundHistory.forEach((rh) => {
    ...
    rh.bets.forEach((bet) => {
      const player = activePlayers.find((p) => p.id === bet.player_id)
      if (player) {
        runningTotals[player.display_name] = (runningTotals[player.display_name] || 0) + (bet.round_points || 0)
