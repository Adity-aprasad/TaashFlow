...
export function BettingPhase({ room, players, currentPlayer }: GamePhaseProps) {
  ...
  const [allBets, setAllBets] = useState<Array<{ player_id: string; amount: number }>>([])
  ...
  const isActive = !!(currentPlayer && !currentPlayer.is_spectator && !currentPlayer.is_eliminated)
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('broadcast', { event: 'phase:reveal_bets' }, (payload) => {
        const data = payload.payload as { bets: Array<{ player_id: string; amount: number }> }
            ...
            {allBets.map((b, i) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="text-sm text-[var(--color-muted)]">{player?.display_name}</p>
                  <p className="text-2xl font-bold text-[var(--color-gold)]">{b.amount}</p>
                </motion.div>
              )
            })}
          </AnimatePresence>
    ...
    <div className="p-6 space-y-6 max-w-md mx-auto">
      ...
      <div className="flex justify-center">
        <div className="relative w-28 h-28">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <span className="text-2xl font-bold">{Math.ceil(timeLeft / 1000)}s</span>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-center gap-4">
            ...
            <div className="w-20 h-20 rounded-[var(--radius-lg)] bg-[var(--color-surface-2)] border-2 border-[var(--color-gold)] flex items-center justify-center">
              <span className="text-3xl font-bold text-[var(--color-gold)]">{bet}</span>
            </div>
      ...
      {locked && (
        ...
        >
          ...
          <p className="text-sm text-[var(--color-muted)] mt-1">Waiting for others…</p>
        </motion.div>
      )}
